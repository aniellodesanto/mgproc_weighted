...
class RankedMetric():
    ...
    def _captures(self, value1: int, value2: int) -> (bool, bool):
        ...
        @Ani (Nov22, 2019) Modified to implement harmony system
        @Ani (Jan 20, 2020) Hardcoded Weighting: metric[1] has double the weight
        """
        
        #instantiate violation counters
        violation1 = 0
        violation2 = 0
        
        #compute harmony
        for i in range(len(value1)):
            #the external if is to hardcode the fact that the metric in pos [1] has double weight
            if i == 0:
                if value1[i] < value2[i]:
                    violation2 += 1
                #if they tie, they both get a violation
                elif value1[i] == value2[i]:
                    violation1 += 1
                    violation2 += 2
                else:
                    violation1 += 1  #because the metric in position 1 weights double
            else:
                #this was the original harmony implementation, with no weights
                if value1[i] < value2[i]:
                    violation2 += 1
                #if they tie, they both get a violation
                elif value1[i] == value2[i]:
        ...
        #decide metric success based on number of violations
